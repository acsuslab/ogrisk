...
def trimmed_matrix_dfs_traversal(matrix, node, context):
    if isinstance(node, dict):
        ...
        for key, value in node.items():
            if isinstance(value, (dict, list)):
                trimmed_matrix_dfs_traversal(matrix, value, context)
    elif isinstance(node, list):
        for item in node:
            trimmed_matrix_dfs_traversal(matrix, item, context)
    else:
        pass



def trimmed_matrix_building_pass(matrix, ast, context):
    trimmed_matrix_dfs_traversal(matrix, ast, context)