...
def maugf_dfs_traversal(node, idcounter):
    ...
    if isinstance(node, dict):
        ...
        for key, value in node.items():
            if isinstance(value, (dict, list)):
                maugf_dfs_traversal(value, idcounter)
    elif isinstance(node, list):
        for item in node:
            maugf_dfs_traversal(item, idcounter)
    else:
        pass


def ast_augmenter_full(ast):
    counter = list()
    counter.append(0)
    maugf_dfs_traversal(ast, counter)
    return counter[0]
