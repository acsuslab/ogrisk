...
def smart_tuples_dfs_traversal(matrix, node, context):
    if isinstance(node, dict):
        ...
        if "type" in node.keys():
            ...
            if "ogrisk_id" in node.keys():
                ...
                s = ""
                if node["type"][0:7] == "ogrisk_":
                    s = str(node["type"]) + str(context.parent_type) + str(node["ogrisk_value"])
                else:     
        ...
        for key, value in node.items():
            if isinstance(value, (dict, list)):
                smart_tuples_dfs_traversal(matrix, value, context)
    elif isinstance(node, list):
        for item in node:
            smart_tuples_dfs_traversal(matrix, item, context)
    else:
        pass



def smart_tuples_building_pass(matrix, ast, context):
    smart_tuples_dfs_traversal(matrix, ast, context)
